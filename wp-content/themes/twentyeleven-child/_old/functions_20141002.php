...
function wt_attendee_details( $checkout ) {
    ...
    if($attendee_count > 0) {
        ...
        echo '<h3 style="clear: both; float: left; width: 100%;">'.__('Please list course attendees\' names and email addresses below.').'</h3>';
...
function wt_attendee_fields_process() {
    ...
    if($error) {
        $woocommerce->add_error( __('Please enter the course attendee details.') );
    }
...
function wt_attendee_update_order_meta( $order_id ) {
    ...
    $attendee_count = wt_count_attendees();
    for($n = 1; $n <= $attendee_count; $n++ ) {
        if ($_POST['attendee_name_'.$n]) update_post_meta( $order_id, $n.' Attendee '.$n.' Name', esc_attr($_POST['attendee_name_'.$n]));
