...
if($_SERVER['REMOTE_ADDR']=='70.178.122.202' || $_SERVER['REMOTE_ADDR']=='81.56.223.186'){

//apply_filters('wc_dynamic_pricing_use_discount_format', '__return_false');

//add_action( 'woocommerce_before_cart', 'check_discount_valid' );
//add_filter('woocommerce_dynamic_pricing_is_rule_set_valid_for_user', 'woocommerce_dynamic_pricing_coupon_is_rule_set_valid_for_user', 10, 3);
/*function check_discount_valid() {
    global $woocommerce;

    if($woocommerce->cart->has_discount('spreadthenews')==1){ //echo 'yes';
        add_filter('woocommerce_dynamic_pricing_process_product_discounts', 1, 10, 4);
        //add_filter('woocommerce_dynamic_pricing_is_rule_set_valid_for_user', 'woocommerce_dynamic_pricing_on', 10, 3);

    } else {                                                    //         echo 'no';
        add_filter('woocommerce_dynamic_pricing_process_product_discounts', 0, 10, 4);
        //add_filter('woocommerce_dynamic_pricing_is_rule_set_valid_for_user', 0, 10, 3);

        //unset(WC_Dynamic_Pricing());
        //add_filter("woocommerce_dynamic_pricing_process_product_discounts",array($this,"false"),10,4);
    }
    //return $result;
} */
...
function is_product_eligible($eligible, $product, $discounter_name, $discounter_object) {
    ...
    if($woocommerce->cart->has_discount('spreadthenews')!=1){  echo 'no';
        $eligible = false;
    } else {echo 'yes';}
    return $eligible;
}
/*
function woocommerce_dynamic_pricing_on(){
    $result = 1;    echo 'teston '.$result;
    return $result;
}
function woocommerce_dynamic_pricing_off(){
    $result = 0;     echo 'testoff '.$result;
    return $result;
} */
}
    ...
    if($attendee_count > 0) {
        ...
        foreach ($woocommerce->cart->get_cart() as $item_id => $values) {
            ...
            //if (get_post_meta($_product->id, '_tribe_wooticket_for_event') > 0)
                ...
                if ( in_array( 2036, $categories ) || in_array( 1377, $categories ) || in_array( 1379, $categories ) ) {
                     $countthis = true;
                }
                if($countthis){
                    ...
                    for($n = 1; $n <= $values['quantity']; $n++ ) {
                        woocommerce_form_field( 'attendee_name_first_'.$n, array(
                            'type'          => 'text',
                        ...
                        $checkout->get_value( 'attendee_name_first_'.$n ));
                        woocommerce_form_field( 'attendee_name_last_'.$n, array(
                            'type'          => 'text',
...
//                            'label'         => __('Attendee '.$n.' Last Name'),
                        ...
                        $checkout->get_value( 'attendee_name_last_'.$n ));
...
//                            'label'         => __('Attendee  '.$n.' Email'),
                        ...
                        $checkout->get_value( 'attendee_title_'.$n ));
                        woocommerce_form_field( 'attendee_title_'.$n, array(
                            'type'          => 'text',
...
function wt_attendee_fields_process() {
    ...
    for($n = 1; $n <= $attendee_count; $n++ ) {
        if (!$_POST['attendee_email_'.$n] || !$_POST['attendee_name_first_'.$n] || !$_POST['attendee_name_last_'.$n])
            $error = true;
            break;
    }
    if($error) {
...
function wt_attendee_update_order_meta( $order_id ) {
    ...
    $attendee_count = wt_count_attendees();
    for($n = 1; $n <= $attendee_count; $n++ ) {
        if ($_POST['attendee_name_first_'.$n]) update_post_meta( $order_id, $n.' Attendee '.$n.' First Name', esc_attr($_POST['attendee_name_first_'.$n]));
        if ($_POST['attendee_name_last_'.$n]) update_post_meta( $order_id, $n.' Attendee '.$n.' Last Name', esc_attr($_POST['attendee_name_last_'.$n]));
        if ($_POST['attendee_email_'.$n]) update_post_meta( $order_id, $n.' Attendee '.$n.'Email', esc_attr($_POST['attendee_email_'.$n]));
        if ($_POST['attendee_title_'.$n]) update_post_meta( $order_id, $n.' Attendee '.$n.'Title', esc_attr($_POST['attendee_title_'.$n]));
    }
