<?php
/*
Plugin Name: Automatic Plugin Updates modified by RED
Plugin URI: https://www.whitefirdesign.com/automatic-plugin-updates
Description: Enables automatic background updates of plugins. Supports INCLUDED selected plugins to be automatically updated. (Functionality inversed by RED.)
Version: 1.0.2
Author: White Fir Design - modified by RED
...
//Check if plugin to be automatically updated is set to be included from automatic updates
function check_if_included_plugin ($update,$item) {

	if (get_option('automatic_plugin_updates_included_plugins'))
		$included_plugins = get_option('automatic_plugin_updates_included_plugins');
	else
		return true;
    ...
    if (in_array($plugin_to_check,$included_plugins))
        return true;
    else
		return false;
}

//Enable automatic plugin updates while checking for included plugins
add_filter( 'auto_update_plugin', 'check_if_included_plugin', 10, 2 );
...
function automatic_plugin_updates_page() {
	...
	if ( ($_SERVER['REQUEST_METHOD'] === 'POST') && wp_verify_nonce($_POST['automatic_plugin_updates'],'automatic_plugin_updates') ) {
		...
		//Store included plugins
		$included_plugins = array();
		foreach ($_POST as  $key => $val) {
			if (strpos($key,'_php'))
				$included_plugins[] = str_replace ('_php','.php',$key);
		}	
		update_option( 'automatic_plugin_updates_included_plugins', $included_plugins, '', 'yes' );
		
		//Store email preference
		update_option( 'automatic_plugin_updates_send_emails', $_POST['send-email'], '', 'yes' );
	}
	else if (get_option('automatic_plugin_updates_included_plugins')) {
		$included_plugins = get_option('automatic_plugin_updates_included_plugins');
	}
	...
	echo '<tr valign="top">';
	echo '<th scope="row">'.__('Enable Automatic Plugin Updates For', 'automatic-plugin-updates').'</th>';
	echo '<td><fieldset>';
	...
	foreach ($current_plugins as  $key => $val) {
		...
		echo $key;
		echo '"';
		if (isset($included_plugins) && in_array ($key, $included_plugins))
			echo 'checked ';
		echo '><label for="';
		echo $key;
		echo '">';
