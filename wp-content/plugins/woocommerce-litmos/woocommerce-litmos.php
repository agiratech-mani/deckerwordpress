 ...
 * Plugin URI: http://www.woothemes.com/products/litmos/
...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '4.4.0', __( 'WooCommerce Litmos', 'woocommerce-litmos' ), __FILE__, 'init_woocommerce_litmos', array(
...
class WC_Litmos extends SV_WC_Plugin {
	...
	public function export_order( $order_id ) {
		...
		if ( ! get_post_meta( $order_id, '_wc_litmos_order_exported', true )  )  {

			$handler = new WC_Litmos_Handler( $order_id );

			$handler->export();

			update_post_meta( $order_id, '_wc_litmos_order_exported', 1 );
		}
	}
	...
	/**
	 * Backwards compat for changing the visibility of admin instance.
	 *
	 * @TODO Remove this as part of WC 2.7 compat {BR 2016-05-23}
	 *
	 * @since 1.6.0
	 */
	public function __get( $name ) {
		...
		if ( 'admin' === $name ) {
			...
			_deprecated_function( 'wc_litmos()->admin', '1.6.0', 'wc_litmos()->get_admin_instance()' );
	...
	public function load_translation() {
		...
		load_plugin_textdomain( 'woocommerce-litmos', false, dirname( plugin_basename( $this->get_file() ) ) . '/i18n/languages' );
	}
	...
	public function get_documentation_url() {
		return 'http://docs.woothemes.com/document/litmos/';
	}
